...
export function FlightRow({ flightNumber, route, times, blockTime, isDeadhead }: FlightRowProps) {
  return (
    <div className="grid grid-cols-[50px_1fr_auto] sm:grid-cols-[50px_72px_1fr_auto] items-baseline gap-x-2 text-[13px]">
      ...
      {blockTime && (
        <span className={`font-medium tabular-nums whitespace-nowrap ${isDeadhead ? 'text-red-500' : 'text-[#3A3780]/60'}`}>{blockTime}</span>
      )}
    </div>
  );
}
