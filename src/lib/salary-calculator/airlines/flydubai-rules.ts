/**
 * Flydubai pure rules — constants, patterns, and validation helpers.
 *
 * Extracted from `flydubai-config.ts` so the cyclic peers
 * (`flight-classifier`, `flydubai-parser`, `csv-parser`) can import them
 * without re-entering `flydubai-config`, which still owns the assembled
 * `FLYDUBAI_CONFIG` (and therefore transitively references
 * `FlydubaiCSVParser`). Mirrors the skywage-mobile Phase 4 Slice 1 fix
 * applied to `src/lib/salary-engine/airlines/flydubai-rules.ts`.
 */

import { CSVColumnMapping, CSVValidationRules } from '@/types/airline-config';
